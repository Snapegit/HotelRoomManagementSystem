...
<div>
	<div :style='{"padding":"20px","margin":"64px auto 10px","borderRadius":"0 0 30px 30px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"15px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/ruzhupingjia"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"padding":"0 10%","margin":"10px auto","alignItems":"flex-start","flexWrap":"wrap","background":"#F6F6F6","display":"flex","width":"100%","position":"relative"}'>
		<div class="attr" :style='{"minHeight":"550px","width":"45%","padding":"0 20px","position":"relative","background":"#fff","display":"flex"}'>
			...
			<div class="info" :style='{"padding":"0","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 0px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>评价内容</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"auto"}'>{{detail.pingjianeirong}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 0px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 0px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>评价时间</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"auto"}'>{{detail.pingjiashijian}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('ruzhupingjia','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFDFA680","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('ruzhupingjia','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFDFA680","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('ruzhupingjia','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      nextClick(page) {
		...
		backClick(){
			...
			this.$router.push({path: '/index/ruzhupingjia', query: params});
		},
		// 下载
		download(file){
			if(!file) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/ruzhupingjiaAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此入住评价？')
			  .then(_ => {
			    this.$http.post('ruzhupingjia/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
