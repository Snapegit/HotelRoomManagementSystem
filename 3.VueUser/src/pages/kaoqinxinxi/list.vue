...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>员工姓名：</div>
        <el-input v-model="formSearch.yuangongxingming" placeholder="员工姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('kaoqinxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"10px","background":"#FFDFA6","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/kaoqinxinxiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"110px","padding":"0 10px 0 0","flex":"1"}'>
				<div class="title">{{item.yuangongxingming}}</div>
				<div class="title">登记时间:{{item.dengjishijian}}</div>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/yuangong/yuangongzhanghao').then(res => {
			if (res.data.code == 0) {
				this.yuangongzhanghaoOptions = res.data.data;
				this.selectOptionsList.push({name:'员工账号',list:this.yuangongzhanghaoOptions,tableName: 'yuangongzhanghao',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.yuangongzhanghao != '') searchWhere.yuangongzhanghao = this.formSearch.yuangongzhanghao;
        if (this.formSearch.yuangongxingming != '') searchWhere.yuangongxingming = '%' + this.formSearch.yuangongxingming + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`kaoqinxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/kaoqinxinxiDetail', query: params});
      },
