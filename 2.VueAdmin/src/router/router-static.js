import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import fangjianxuding from '@/views/modules/fangjianxuding/list'
    import news from '@/views/modules/news/list'
    import yuangong from '@/views/modules/yuangong/list'
    import aboutus from '@/views/modules/aboutus/list'
    import fangjianxinxi from '@/views/modules/fangjianxinxi/list'
    import fangjianleixing from '@/views/modules/fangjianleixing/list'
    import fangjianruzhu from '@/views/modules/fangjianruzhu/list'
    import ruzhupingjia from '@/views/modules/ruzhupingjia/list'
    import fangjiantuifang from '@/views/modules/fangjiantuifang/list'
    import fangjianyuyue from '@/views/modules/fangjianyuyue/list'
    import yonghu from '@/views/modules/yonghu/list'
    import discussnews from '@/views/modules/discussnews/list'
    import discussfangjianxinxi from '@/views/modules/discussfangjianxinxi/list'
    import kaoqinxinxi from '@/views/modules/kaoqinxinxi/list'
    import config from '@/views/modules/config/list'
    import newstype from '@/views/modules/newstype/list'


//2.配置路由   注意：名字
export const routes = [{
    path: '/',
    name: '系统首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '系统首页',
      component: Home,
      meta: {icon:'', title:'center', affix: true}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/fangjianxuding',
        name: '房间续订',
        component: fangjianxuding
      }
      ,{
	path: '/news',
        name: '酒店资讯',
        component: news
      }
      ,{
	path: '/yuangong',
        name: '员工',
        component: yuangong
      }
      ,{
	path: '/aboutus',
        name: '关于我们',
        component: aboutus
      }
      ,{
	path: '/fangjianxinxi',
        name: '房间信息',
        component: fangjianxinxi
      }
      ,{
	path: '/fangjianleixing',
        name: '房间类型',
        component: fangjianleixing
      }
      ,{
	path: '/fangjianruzhu',
        name: '房间入住',
        component: fangjianruzhu
      }
      ,{
	path: '/ruzhupingjia',
        name: '入住评价',
        component: ruzhupingjia
      }
      ,{
	path: '/fangjiantuifang',
        name: '房间退房',
        component: fangjiantuifang
      }
      ,{
	path: '/fangjianyuyue',
        name: '房间预约',
        component: fangjianyuyue
      }
      ,{
	path: '/yonghu',
        name: '用户',
        component: yonghu
      }
      ,{
	path: '/discussnews',
        name: '酒店资讯',
        component: discussnews
      }
      ,{
	path: '/discussfangjianxinxi',
        name: '房间信息评论',
        component: discussfangjianxinxi
      }
      ,{
	path: '/kaoqinxinxi',
        name: '考勤信息',
        component: kaoqinxinxi
      }
      ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
      ,{
	path: '/newstype',
        name: '酒店资讯分类',
        component: newstype
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})
const originalPush = VueRouter.prototype.push
//修改原型对象中的push方法
VueRouter.prototype.push = function push(location) {
   return originalPush.call(this, location).catch(err => err)
}
export default router;
