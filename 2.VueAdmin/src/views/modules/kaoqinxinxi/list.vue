<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 10px 0 0","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select" label="员工账号" prop="yuangongzhanghao">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">员工账号</label>
						<el-select clearable v-model="searchForm.yuangongzhanghao" placeholder="请选择员工账号" >
							<el-option v-for="(item,index) in yuangongzhanghaoOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">员工姓名</label>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('kaoqinxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('kaoqinxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu1" :style='{"margin":"0 2px","fontSize":"14px","color":"#ffffff","fontWeight":"600","height":"40px"}'></span>
						删除
					</el-button>


					<download-excel v-if="isAuth('kaoqinxinxi','导出')" class="export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "考勤信息.xls">
						<!-- 导出excel -->
						<el-button class="btn18" type="success">
							<span class="icon iconfont icon-daochu4" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
							导出
						</el-button>
					</download-excel>

					<el-button class="btn18" v-if="isAuth('kaoqinxinxi','出勤统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kaoqinxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('kaoqinxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kaoqinxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yuangongxingmingChart1" style="width:100%;height:600px;"></div>
...
//导出excel
				json_fields: {
					"员工账号": "yuangongzhanghao",    //常规字段
					"员工姓名": "yuangongxingming",    //常规字段
					"出勤天数": "chuqintianshu",    //常规字段
					"请假天数": "qingjiatianshu",    //常规字段
					"迟到次数": "chidaocishu",    //常规字段
					"早退次数": "zaotuicishu",    //常规字段
					"缺勤天数": "queqintianshu",    //常规字段
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var yuangongxingmingChart1 = echarts.init(document.getElementById("yuangongxingmingChart1"),'macarons');
        this.$http({
            url: `kaoqinxinxi/valueMul/yuangongxingming?yColumnNameMul=chuqintianshu,chidaocishu,zaotuicishu,queqintianshu`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yuangongxingmingChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yuangongxingmingChart1.resize();
                };
            }
        });
      })
	...
	kaoqinxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kaoqinxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
