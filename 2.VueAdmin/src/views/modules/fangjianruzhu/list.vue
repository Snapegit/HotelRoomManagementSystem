<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 10px 0 0","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">房间名称</label>
						<el-input v-model="searchForm.fangjianmingcheng" placeholder="房间名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select" label="入住状态" prop="ruzhuzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">入住状态</label>
						<el-select clearable v-model="searchForm.ruzhuzhuangtai" placeholder="请选择入住状态" >
							<el-option v-for="(item,index) in ruzhuzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('fangjianruzhu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('fangjianruzhu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="statis" v-if="isAuth('fangjianruzhu','报表')" type="success" @click="chartDialog()">
						<span class="icon iconfont icon-tongji7" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						统计报表
					</el-button>
				</el-row>
			</el-form>
			<div :style='{"width":"100%","padding":"0"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fangjianruzhu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('fangjianruzhu','退房')" @click="fangjiantuifangCrossAddOrUpdateHandler(scope.row,'cross','','','ruzhuzhuangtai','已退','已退房,未退房'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#212529","height":"40px"}'></span>
								退房
							</el-button>
							<el-button class="btn8" v-if="isAuth('fangjianruzhu','房间续订')" @click="fangjianxudingCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#212529","height":"40px"}'></span>
								房间续订
							</el-button>
							<el-button class="edit" v-if=" isAuth('fangjianruzhu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('fangjianruzhu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<fangjiantuifang-cross-add-or-update v-if="fangjiantuifangCrossAddOrUpdateFlag" :parent="this" ref="fangjiantuifangCrossaddOrUpdate"></fangjiantuifang-cross-add-or-update>
		<fangjianxuding-cross-add-or-update v-if="fangjianxudingCrossAddOrUpdateFlag" :parent="this" ref="fangjianxudingCrossaddOrUpdate"></fangjianxuding-cross-add-or-update>
...
import fangjiantuifangCrossAddOrUpdate from "../fangjiantuifang/add-or-update";
import fangjianxudingCrossAddOrUpdate from "../fangjianxuding/add-or-update";
	export default {
		data() {
			return {
				...
				fangjiantuifangCrossAddOrUpdateFlag: false,
				fangjianxudingCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			fangjiantuifangCrossAddOrUpdate,
			fangjianxudingCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			fangjiantuifangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fangjiantuifangCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fangjianruzhu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fangjiantuifangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fangjiantuifangCrossaddOrUpdate.init(row.id,type);
				});
			},
			fangjianxudingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fangjianxudingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fangjianruzhu');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fangjianxudingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fangjianxudingCrossaddOrUpdate.init(row.id,type);
				});
			},
    chartDialog() {
	...
	fangjianruzhustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'fangjianruzhu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
