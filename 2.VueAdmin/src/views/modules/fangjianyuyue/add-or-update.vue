	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="入住日期" prop="ruzhuriqi">
					<el-date-picker
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.ruzhuriqi" label="入住日期" prop="ruzhuriqi">
					<el-input v-model="ruleForm.ruzhuriqi" placeholder="入住日期" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="结束日期" prop="jieshuriqi">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.jieshuriqi" label="结束日期" prop="jieshuriqi">
					<el-input v-model="ruleForm.jieshuriqi" placeholder="结束日期" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="预约状态" prop="yuyuezhuangtai">
					<el-select :disabled="ro.yuyuezhuangtai" v-model="ruleForm.yuyuezhuangtai" placeholder="请选择预约状态" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="预约状态" prop="yuyuezhuangtai">
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				this.ruleForm.yuyuezhuangtai = '未入住'; 			}
			// 获取用户信息
			this.$http({
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `fangjianyuyue/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fangjianyuyueCrossAddOrUpdateFlag = false;
											this.parent.search();
