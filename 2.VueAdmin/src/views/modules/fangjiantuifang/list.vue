<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('fangjiantuifang','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('fangjiantuifang','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu1" :style='{"margin":"0 2px","fontSize":"14px","color":"#ffffff","fontWeight":"600","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('fangjiantuifang','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
						批量支付
					</el-button>

					<el-button class="btn18" v-if="isAuth('fangjiantuifang','日营业额')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('fangjiantuifang','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('fangjiantuifang','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fangjiantuifang','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('fangjiantuifang','评价')" @click="ruzhupingjiaCrossAddOrUpdateHandler(scope.row,'cross','否','是','[1]','已评价')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#212529","height":"40px"}'></span>
								评价
							</el-button>
							<el-button class="edit" v-if=" isAuth('fangjiantuifang','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('fangjiantuifang','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<ruzhupingjia-cross-add-or-update v-if="ruzhupingjiaCrossAddOrUpdateFlag" :parent="this" ref="ruzhupingjiaCrossaddOrUpdate"></ruzhupingjia-cross-add-or-update>
...
import ruzhupingjiaCrossAddOrUpdate from "../ruzhupingjia/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				ruzhupingjiaCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			ruzhupingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.ruzhupingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fangjiantuifang');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.ruzhupingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.ruzhupingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','fangjiantuifang');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: `fangjiantuifang/value/tuifangriqi/dingdanjine/日`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].tuifangriqi);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].tuifangriqi
                    })
                }
                var option = {};
                option = {
	...
	fangjiantuifangstatusChange(e,row){
		if(row.status==0){
