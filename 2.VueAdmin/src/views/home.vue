...
<div class="content" :style='{"minHeight":"100vh","padding":"30px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20240220/927b2d08dcdb48b3af07cef8d1a59f59.png) no-repeat center top / cover","display":"flex","width":"100%"}'>
	...
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":"1px solid  #f0f0f0","margin":"0 10px","borderRadius":"4px","background":"#ffffff","display":"flex"}' v-if="isAuth('fangjianruzhu','首页总数')">
			...
			<div :style='{"width":"160px","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{fangjianruzhuCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>房间入住总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgb(240, 240, 240)","margin":"0 10px","borderRadius":"4px","background":"rgb(255, 255, 255)","display":"flex"}' v-if="isAuth('fangjiantuifang','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-tongji5" :style='{"color":"rgb(247, 179, 107)","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{fangjiantuifangCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>房间退房总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 1 -->
	<div class="type1" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","display":"flex","width":"100%","position":"relative","justifyContent":"center","height":"auto"}'>
		<div id="fangjiantuifangChart1" class="echarts1" v-if="isAuth('fangjiantuifang','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
            fangjianruzhuCount: 0,
            fangjiantuifangCount: 0,
	...
	mounted(){
		this.init();
		this.getfangjianruzhuCount();
		this.getfangjiantuifangCount();
		this.fangjiantuifangChat1();
	},
	methods:{
		// 统计图动画
		...
		getfangjianruzhuCount() {
			this.$http({
				url: `fangjianruzhu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.fangjianruzhuCount = data.data
				}
			})
		},
		getfangjiantuifangCount() {
			this.$http({
				url: `fangjiantuifang/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.fangjiantuifangCount = data.data
				}
			})
		},
// 1234 竖
		fangjiantuifangChat1() {
			this.$nextTick(()=>{

        var fangjiantuifangChart1 = echarts.init(document.getElementById("fangjiantuifangChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                fangjiantuifangChart1.setOption(option);
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, fangjiantuifangChart1)
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    fangjiantuifangChart1.resize();
                };
            }
        });
      })
